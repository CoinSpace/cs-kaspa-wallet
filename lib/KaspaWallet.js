...
export default class KaspaWallet extends CsWallet {
  ...
  constructor(options = {}) {
    ...
    this.#prepareTransaction = this.memoize(this._prepareTransaction);
  }
  ...
  async cleanup() {
    ...
    this.memoizeClear(this.#prepareTransaction);
    this.#api.cleanup();
  }
  ...
  getPrivateKey(seed) {
    ...
    for (const utxo of this.#utxos) {
      if (exported[utxo.address]) continue;
  ...
  #getAddress(change = false) {
    ...
    const lastUsedAddressIndex = Math.max(-1, ...[...this.#addresses.values()]
      .filter((item) => item.bip32DerivationPath[0] === index && item.used === true)
      .map((item) => item.bip32DerivationPath[1]));
    const key = this.#account
      .deriveChild(index)
      .deriveChild(lastUsedAddressIndex + 1);
    const address = this.#getAddressFromPublicKey(key.publicKeySchnorr);
    if (!this.#addresses.has(address)) {
      this.#addresses.set(address, {
        bip32DerivationPath: [index, lastUsedAddressIndex + 1],
        used: false,
      });
    }
    return address;
  ...
  async #discovery() {
    ...
    for (const change of [0, 1]) {
      ...
      while (gap < BIP44_GAP_LIMIT) {
        ...
        for (const address of batch) {
          ...
          if (info?.active) {
            gap = 0;
            this.#addresses.get(address).used = true;
          } else {
            gap++;
          }
        }
    ...
    if (this.#usedAddresses.length) {
      ...
      this.#utxos = utxos.map((item) => {
        return {
          ...item,
          bip32DerivationPath: this.#addresses.get(item.address).bip32DerivationPath,
        };
      });
    }
  }
  ...
  async _estimateMaxAmount({ feeRate, price }) {
  ...
  async createTransaction({ feeRate, address, amount, price }, seed, rand) {
    ...
    tx.outputs.forEach((output, index) => {
      if (this.#addresses.has(output.address)) {
        this.#addresses.get(output.address).used = true;
        this.#utxos.push({
          ...
          bip32DerivationPath: this.#addresses.get(output.address).bip32DerivationPath,
        });
      }
    });
  ...
  async _prepareTransaction({ address, value, feeRate, price }) {
