...
export default class KaspaWallet extends CsWallet {
  ...
  get #lastUsedAddressIndexes() {
    return [
      Math.max(-1, ...[...this.#addresses.values()]
        .filter((item) => item.bip32DerivationPath[0] === 0 && item.used === true)
        .map((item) => item.bip32DerivationPath[1])),
      Math.max(-1, ...[...this.#addresses.values()]
        .filter((item) => item.bip32DerivationPath[0] === 1 && item.used === true)
        .map((item) => item.bip32DerivationPath[1])),
    ];
  }
